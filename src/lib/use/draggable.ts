import type { PointerType, Position } from '$lib/types';
import { clamp, isClient } from '$lib/utils/is';
import { toValue, type Func } from '$lib/utils/values';
...
export const emptyDragState: Draggable = {
	isLocked: false,
	isDragging: false,
	position: { x: 0, y: 0 },
	style: ''
...
export interface DraggableOptions {
	 ...
	 */
	onUpdate?: (state: Draggable) => void | false;

	/**
	 * Callback when the dragging starts. Return `false` to prevent dragging.
	 */
	onStart?: (state: Draggable, event: PointerEvent) => void | false;

	/**
	 * Callback during dragging.
	 */
	onMove?: (state: Draggable, event: PointerEvent) => void;

	/**
	 * Callback when dragging end.
	 */
	onEnd?: (state: Draggable, event: PointerEvent) => void;
...
> {
	...
	const store = writable<Draggable>(
		{
			isLocked: false,
			isDragging: false,
		...
		function start(set, update) {
			...
			function handleUpdate(callback: (state: Draggable) => Partial<Draggable> | void) {
				update((draggable) => {
